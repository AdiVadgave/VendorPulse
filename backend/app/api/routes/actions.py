...
def add_action(cycleId: str, payload: ActionItemCreate, action_repo=Depends(get_action_repo)):
    ...
    logger.info("ACTIONS: added %s to cycle %s (source=%s)", sanitize_for_log(record["action_id"]), sanitize_for_log(cycleId), sanitize_for_log(record["source"]))
...
def add_actions_bulk(cycleId: str, payload: ActionItemBulkCreate, action_repo=Depends(get_action_repo)):
    ...
    logger.info(
        ...
        len(payload.actions), len(added), len(payload.actions) - len(added), consolidated, sanitize_for_log(cycleId),
    )
...
def update_action(
    ...
    logger.info("ACTIONS: updated %s (%s)", sanitize_for_log(actionId), sanitize_for_log(", ".join(updates.keys())))
...
def delete_action(cycleId: str, actionId: str, action_repo=Depends(get_action_repo)):
    ...
    logger.info("ACTIONS: deleted %s from cycle %s", sanitize_for_log(actionId), sanitize_for_log(cycleId))
