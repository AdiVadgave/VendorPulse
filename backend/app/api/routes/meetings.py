...
def list_meetings(
    ...
    logger.info("list_meetings called — cycleId=%s", sanitize_for_log(cycleId))
...
def create_meeting(
    ...
    logger.info(
        ...
        sanitize_for_log(payload.title), sanitize_for_log(payload.organizer_id), sanitize_for_log(payload.participant_ids), sanitize_for_log(payload.time_slot),
    )
    try:
        meeting, warnings = svc.create_meeting(payload)
    except ValueError as exc:
        logger.warning("create_meeting validation error: %s", exc)
        raise HTTPException(status_code=400, detail="Invalid meeting payload")
...
def get_meeting(meetingId: str, svc: MeetingService = Depends(get_meeting_service)):
    logger.info("get_meeting called — meetingId=%s", sanitize_for_log(meetingId))
    meeting = svc.get_meeting(meetingId)
    if meeting is None:
        logger.warning("get_meeting: meeting %s not found", sanitize_for_log(meetingId))
...
def update_meeting(
    ...
    logger.info("update_meeting called — meetingId=%s, payload=%s", sanitize_for_log(meetingId), sanitize_for_log(payload.model_dump(exclude_none=True)))
    meeting = svc.update_meeting(meetingId, payload)
    if meeting is None:
        logger.warning("update_meeting: meeting %s not found", sanitize_for_log(meetingId))
        raise HTTPException(status_code=404, detail="Meeting not found")
    logger.info("update_meeting success — meetingId=%s", sanitize_for_log(meetingId))
    return {"meeting": meeting}
...
def cancel_meeting(
    ...
    logger.info("cancel_meeting called — meetingId=%s, organizer_id=%s", sanitize_for_log(meetingId), sanitize_for_log(payload.organizer_id))
    try:
    ...
    if meeting is None:
        logger.warning("cancel_meeting: meeting %s not found", sanitize_for_log(meetingId))
        raise HTTPException(status_code=404, detail="Meeting not found")
    logger.info("cancel_meeting success — meetingId=%s", sanitize_for_log(meetingId))
...
def respond_to_meeting(
    ...
    logger.info("respond_to_meeting called — meetingId=%s, user_id=%s, status=%s", sanitize_for_log(meetingId), sanitize_for_log(payload.user_id), sanitize_for_log(payload.status))
    try:
        ...
        raise HTTPException(status_code=403, detail="Not allowed to respond to this meeting")
    if meeting is None:
        logger.warning("respond_to_meeting: meeting %s not found", sanitize_for_log(meetingId))
        raise HTTPException(status_code=404, detail="Meeting not found")
    logger.info("respond_to_meeting success — meetingId=%s, status=%s", sanitize_for_log(meetingId), sanitize_for_log(payload.status))
