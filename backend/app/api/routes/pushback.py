...
def add_pushback(cycleId: str, payload: PushbackCreate, repo=Depends(get_pushback_repo)):
    ...
    repo.insert(record)
    logger.info("PUSHBACK: added %s to cycle %s", sanitize_for_log(record["pushback_id"]), sanitize_for_log(cycleId))
...
def replace_responses(
    ...
    logger.info("PUSHBACK: saved %d response(s) for %s", len(saved), sanitize_for_log(pushbackId))
