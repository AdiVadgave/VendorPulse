...
class BaseRepository:
    ...
    def update_by_id(self, id_field: str, id_value: Any, updates: dict) -> Optional[dict]:
        ...
        with get_pool().connection() as conn:
            ...
            row = cur.fetchone()
        if row is None:
            logger.warning("update_by_id: not found — %s=%s in %s", sanitize_for_log(id_field), sanitize_for_log(id_value), sanitize_for_log(self.table))
            return None
    ...
    def replace_by_id(self, id_field: str, id_value: Any, new_record: dict) -> Optional[dict]:
        ...
        with get_pool().connection() as conn:
            ...
            row = cur.fetchone()
        if row is None:
            logger.warning("replace_by_id: not found — %s=%s in %s", sanitize_for_log(id_field), sanitize_for_log(id_value), sanitize_for_log(self.table))
            return None
        return self._row_to_dict(row)
