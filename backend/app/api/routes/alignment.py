...
def extract_actions(cycleId: str, payload: ExtractActionsRequest):
    ...
    """
    logger.info("ALIGNMENT: extract actions — cycleId=%s, text_len=%d", sanitize_for_log(cycleId), len(payload.notes_text))
...
def schedule_alignment_meeting_manual(
    ...
    logger.info("ALIGNMENT: manual meeting — cycleId=%s, index=%s", sanitize_for_log(cycleId), sanitize_for_log(payload.meeting_index))
    ...
    try:
        ...
        if is_reschedule:
            ...
            logger.info("ALIGNMENT: manual meeting rescheduled — meeting_id=%s", sanitize_for_log(event_id))
        else:
            meeting_repo.insert(meeting_record)
            logger.info("ALIGNMENT: manual meeting persisted — meeting_id=%s", sanitize_for_log(event_id))
        ...
        logger.exception("ALIGNMENT: failed to persist meeting for cycle=%s", sanitize_for_log(cycleId))
        raise HTTPException(status_code=500, detail="Failed to persist alignment meeting")
...
def delete_alignment_meeting(
    ...
    if event_id and token:
        try:
            ...
            logger.warning("ALIGNMENT: Graph cancel failed for %s: %s", sanitize_for_log(event_id), e)
    ...
    logger.info("ALIGNMENT: deleted meeting index=%s (event=%s, cancelled=%s)", sanitize_for_log(index), sanitize_for_log(event_id), cancelled)
...
def _strip_markdown_json(text: str) -> str:
    ...
    # Avoid broad greedy regex on untrusted text; deterministically extract the
    # widest JSON-looking object/array span instead.
