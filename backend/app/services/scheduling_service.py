...
class SchedulingService:
    ...
    def get_attendees(self, cycle_id: str, *, seed_from_previous: bool = False) -> list[dict]:
        ...
        """
        logger.info("get_attendees — cycle_id=%s, seed_from_previous=%s", sanitize_for_log(cycle_id), seed_from_previous)
        current_attendees = self._attendees.get_for_cycle(cycle_id)
        if current_attendees:
            logger.info("get_attendees: found %d existing attendees for cycle %s", len(current_attendees), sanitize_for_log(cycle_id))
        ...
        logger.info("get_attendees: seeded %d attendees from previous cycle into cycle %s", len(new_attendees), sanitize_for_log(cycle_id))
    ...
    def send_attendance_outreach(
        ...
        Stores reply-tracking fields on each attendee record when Graph is used:
          ...
          - outreach_sent_at
        """
        logger.info("send_attendance_outreach — cycle_id=%s, graph_service=%s", sanitize_for_log(cycle_id), "present" if graph_service else "None")
        attendees = self._attendees.get_for_cycle(cycle_id)
    ...
    def approve_slot(
        ...
        """
        logger.info("approve_slot — cycle_id=%s, slot_id=%s, approved_by=%s, time_zone=%s", sanitize_for_log(cycle_id), sanitize_for_log(slot_id), sanitize_for_log(approved_by), sanitize_for_log(time_zone))
    ...
    def send_invites(self, cycle_id: str, slot_id: str, organiser_id: str) -> AgentResponse:
        ...
        """
        logger.info("send_invites — cycle_id=%s, slot_id=%s, organiser_id=%s", sanitize_for_log(cycle_id), sanitize_for_log(slot_id), sanitize_for_log(organiser_id))
        slot = self._slots.get_by_slot_id(slot_id)
        if not slot:
            return AgentResponse(
                status="failed",
    ...
    def get_rsvp_status(self, cycle_id: str) -> AgentResponse:
        ...
        """
        logger.info("get_rsvp_status — cycle_id=%s", sanitize_for_log(cycle_id))
        attendees = self._attendees.get_for_cycle(cycle_id)
    ...
    def update_rsvp(self, cycle_id: str, attendee_id: str, status: str) -> Optional[dict]:
        ...
        logger.info("update_rsvp — cycle_id=%s, attendee_id=%s, status=%s", sanitize_for_log(cycle_id), sanitize_for_log(attendee_id), sanitize_for_log(status))
