...
// Draft bodies interpolate app/user-influenced data (vendor names, backend-
// generated content) into HTML that is later rendered via dangerouslySetInnerHTML
// and seeded into a contentEditable. Sanitize with an allow-list so formatting is
// preserved but scripts / event handlers / javascript: URLs can never execute
// (defense against stored/DOM XSS — CWE-79).
...
export default function DraftReviewDialog({
  ...
  function handleSend() {
    ...
    onSend({ subject: subj, body: sanitizeHtml(html) }, dirty)
  }
