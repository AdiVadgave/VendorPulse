...
Slot discovery and ranking are deterministic (see slot_ranking_service). Meeting
records are created locally; calendar/Teams event creation is best-effort via the
delegated Graph token when one is configured, and skipped when it is not.
"""
...
def _check_workflow_state(cycle: dict, required_state: str) -> None:
    ...
    try:
        ...
        logger.warning(
            "workflow-state gate failed — cycle_id=%s required=%s error=%s",
            sanitize_for_log(cycle.get("cycle_id")),
            sanitize_for_log(required_state),
            exc,
        )
        raise HTTPException(status_code=409, detail=f"Cycle must reach '{required_state}' before this action")
...
def get_cycle(cycleId: str, cycle_repo=Depends(get_cycle_repo)):
    logger.info("get_cycle called — cycleId=%s", sanitize_for_log(cycleId))
    ...
    logger.info("get_cycle success — cycleId=%s, workflow_state=%s", sanitize_for_log(cycleId), sanitize_for_log(ws))
    return {"cycle": cycle}
...
def set_workflow_state(
    ...
    logger.info("set_workflow_state — cycleId=%s, %s -> %s", sanitize_for_log(cycleId), sanitize_for_log(current), sanitize_for_log(target))
...
def set_manual_meeting(
    ...
    if current_idx < target_idx:
        for _ in range(target_idx - current_idx):
            updated = workflow_engine.advance(updated, cycle_repo)
    logger.info("MANUAL-MEETING — cycleId=%s scheduled_at=%s tz=%s", sanitize_for_log(cycleId), sanitize_for_log(payload.start_time), sanitize_for_log(payload.time_zone))
    return {
        "cycle": updated,
        "scheduled_at": payload.start_time,
...
def delete_cycle(
    ...
    logger.info("delete_cycle called — cycleId=%s", sanitize_for_log(cycleId))
    ...
    logger.info(
        ...
        sanitize_for_log(cycleId), removed_attendees, removed_actions, removed_meetings, removed_submissions,
    )
    return {
...
def get_attendees(
    ...
    logger.info("get_attendees called — cycleId=%s, seedFromPrevious=%s", sanitize_for_log(cycleId), seedFromPrevious)
    attendees = svc.get_attendees(cycleId, seed_from_previous=seedFromPrevious)
    logger.info("get_attendees returning %d attendees for cycleId=%s", len(attendees), sanitize_for_log(cycleId))
...
def add_attendees(
    ...
    logger.info("add_attendees called — cycleId=%s, count=%d", sanitize_for_log(cycleId), len(attendees))
    _get_cycle_or_404(cycleId, cycle_repo)   # ensure cycle exists
    result = svc.add_attendees(cycleId, attendees)
    logger.info("add_attendees success — cycleId=%s", sanitize_for_log(cycleId))
    return result
