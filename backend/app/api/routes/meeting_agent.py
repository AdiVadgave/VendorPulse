...
def generate_minutes(
    ...
    logger.info(
        ...
        sanitize_for_log(cycleId), sanitize_for_log(payload.meeting_id), len(payload.notes),
    )
...
def extract_actions(
    ...
    logger.info("MEETING-AGENT: extract actions — cycleId=%s, text_len=%d", sanitize_for_log(cycleId), len(minutes_text))
...
def parse_transcript(
    ...
    logger.info(
        ...
        sanitize_for_log(cycleId), sanitize_for_log(payload.meeting_id), len(payload.transcript),
    )
...
def extract_transcript_file(cycleId: str, payload: TranscriptFileRequest):
    ...
    logger.info("MEETING-AGENT: transcript file extracted — cycleId=%s file=%s chars=%d", sanitize_for_log(cycleId), sanitize_for_log(name), len(text))
...
def send_minutes(cycleId: str, payload: SendMinutesRequest):
    ...
    for attendee in internal:
        ...
        try:
            ...
            logger.info("MEETING-AGENT: minutes sent to %s (%s)", sanitize_for_log(name), sanitize_for_log(email_addr))
        except MailSendError as exc:
            logger.warning("MEETING-AGENT: failed to send to %s — %s", sanitize_for_log(email_addr), exc)
            failed.append({"name": name, "email": email_addr, "error": str(exc)})

    if not sent_to and failed:
        logger.error(
            "MEETING-AGENT: dispatch failed for cycle=%s, first_error=%s",
            sanitize_for_log(cycleId), sanitize_for_log(failed[0]["error"]),
        )
        raise HTTPException(
            status_code=503,
            detail="Mail send failed via the service mailbox. Please verify mail configuration and retry."
        )
